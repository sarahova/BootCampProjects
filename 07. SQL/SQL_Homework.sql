...
SELECT last_name, first_name FROM actor WHERE last_name LIKE '%Li%' ORDER BY last_name ASC;
...
SELECT staff.first_name, staff.last_name, CONCAT('$', FORMAT(SUM(payment.amount),2)) AS 'COUNT OF RENTALS IN AUG 2005' 
...
SELECT film.title, COUNT(inventory.inventory_id) AS 'NUMBER OF COPIES IN INVENTORY' FROM inventory 
INNER JOIN film ON film.film_id=inventory.film_id
WHERE title = 'Hunchback Impossible';

SELECT COUNT(inventory_id) AS 'INVENTORY COUNT' FROM inventory WHERE film_id 
IN (
	SELECT film_id FROM film WHERE title='Hunchback Impossible');

#6e. Using the tables payment and customer and the JOIN command, list the total paid by each customer. List the customers alphabetically by last name.
SELECT customer.first_name, customer.last_name, SUM(payment.amount) AS 'TOTAL PAID' FROM payment
...
SELECT CONCAT(first_name, ' ' , last_name) AS 'ACTOR NAMES IN ALONE TRIP' 
...
SELECT film.title, COUNT(inventory.film_id) AS rental_count FROM rental
...
CREATE VIEW sakila.top_5_genre AS
SELECT c.`name` AS genre, SUM(p.amount) AS revenue FROM payment p
	LEFT JOIN rental r ON r.rental_id=p.rental_id
		LEFT JOIN inventory i ON i.inventory_id = r.inventory_id
			LEFT JOIN film_category fc ON fc.film_id=i.film_id
